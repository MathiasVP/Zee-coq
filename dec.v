...
Class is_dec_eq (A : Type) :=
  {
    dec_eq: forall (a b : A), { a = b } + { a <> b }
  }.

Instance is_dec_eq_nat : is_dec_eq nat.
constructor.
- intros.
  decide equality.
Defined.
Hint Resolve is_dec_eq_nat : typeclass_instances.

Lemma dec_eq_refl {A : Type} `{is_dec_eq A}:
...
Lemma dec_eq_neq {A : Type} `{is_dec_eq A}:
...
Class is_dec_rel {A B : Type} (f : A -> B -> Prop) :=
  {
    dec_rel: forall a b, { f a b } + { ~ f a b }
  }.

Class is_dec_pred {A : Type} (f : A -> Prop) :=
  {
...
Definition Full' {A:Type} (l1 l2:list A) := NoDup l1 /\ NoDup l2 /\ forall a:A, In a l1 <-> ~ In a l2.

Lemma about_Full' {A : Type} `{is_dec_eq A}:
  forall (a : A) xs ys,
    ~ In a ys ->
    Full' ys (a :: xs) ->
    Full' (a :: ys) xs.
...
Lemma listing_implies_full' {A : Type}:
  forall xs,
...
Instance dec_eq_pair (A B : Type) `{is_dec_eq A} `{is_dec_eq B} : is_dec_eq (A * B).
Proof.
  constructor.
  intros [a1 b1] [a2 b2].
...
Lemma full'_nil_implies_listing {A : Type} `{is_dec_eq A}:
...
Lemma full'_symm {A : Type} `{is_dec_eq A}:
...
Instance iff_dec {A : Type} {P Q : A -> Prop}
...
Instance forall_dec {A B : Type} `{Finite B} `{is_dec_eq B} {P : A -> B -> Prop}
...
Instance exist_dec {A B : Type} `{Finite B} `{is_dec_eq B} {P : A -> B -> Prop}
...
Instance in_dec {A : Type} `{is_dec_eq A} {xs : list A} : is_dec_pred (fun a => In a xs).
Proof.
  constructor.
  intros.
